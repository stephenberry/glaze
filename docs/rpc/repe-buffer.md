...
Returns `true` on success. On a parse failure it writes the error response through `state.out` itself, so a handler should return without writing anything further. The exception is a notification, which is answered by silence whether the read succeeds or fails: `read_params` returns `false` having left `state.out` untouched. Returning immediately on `false` is correct in both cases, but a handler that inspects the response buffer afterwards has to allow for it being empty.
