...
When using custom types with `glz::custom` in variants, Glaze automatically infers the JSON type from your read function's parameter type.

## Automatic Type Inference

Glaze examines the second parameter of your `read_fn` lambda to determine how your custom type should be classified:
...
```c++
// Strong typedef for amounts (distinct from percentages)
struct Amount {
   double value{};
};
...
struct glz::meta<Amount> {
   // Glaze infers this is a numeric type from the 'const double&' parameter
...
// Works automatically in variants!
std::variant<std::string, Amount> v;
glz::read_json(v, "42.5");  // Successfully parses as Amount
```
