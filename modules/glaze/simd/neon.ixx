...
export namespace glz::detail
{
   ...
   GLZ_ALWAYS_INLINE void neon_string_escape(const char*& c, const char* e, Data*& data, std::size_t n, WriteEscape&& write_escape)
   {
      if (n > 15) {
         ...
         for (const char* end_m63 = e - 63; c < end_m63;) {
            const uint8x16_t v0 = vld1q_u8(reinterpret_cast<const std::uint8_t*>(c));
            const uint8x16_t v1 = vld1q_u8(reinterpret_cast<const std::uint8_t*>(c + 16));
            const uint8x16_t v2 = vld1q_u8(reinterpret_cast<const std::uint8_t*>(c + 32));
            const uint8x16_t v3 = vld1q_u8(reinterpret_cast<const std::uint8_t*>(c + 48));

            vst1q_u8(reinterpret_cast<std::uint8_t*>(data), v0);
            vst1q_u8(reinterpret_cast<std::uint8_t*>(data + 16), v1);
            vst1q_u8(reinterpret_cast<std::uint8_t*>(data + 32), v2);
            vst1q_u8(reinterpret_cast<std::uint8_t*>(data + 48), v3);
         ...
         for (const char* end_m15 = e - 15; c < end_m15;) {
            const uint8x16_t v = vld1q_u8(reinterpret_cast<const std::uint8_t*>(c));
            vst1q_u8(reinterpret_cast<std::uint8_t*>(data), v); // speculative store
            ...
            while (std::uint8_t(*c) >= 0x20 && std::uint8_t(*c) != '"' && std::uint8_t(*c) != '\\') {
               ++data;
               ++c;
            }
            write_escape();
