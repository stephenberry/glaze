...
namespace glz
{
   ...
   GLZ_ALWAYS_INLINE void byteswap_le(T& value) noexcept
   {
      if constexpr (std::is_enum_v<T>) {
         ...
         using Int = std::conditional_t<sizeof(T) == 4, std::uint32_t, std::uint64_t>;
         Int int_val;
...
{
   export template <class T>
   constexpr std::uint8_t byte_count = std::uint8_t(std::bit_width(sizeof(T)) - 1);

   export inline constexpr std::array<std::uint8_t, 8> byte_count_lookup{1, 2, 4, 8, 16, 32, 64, 128};

   export [[nodiscard]] GLZ_ALWAYS_INLINE constexpr std::size_t int_from_compressed(auto&& ctx, auto&& it, auto end) noexcept
   {
      if (it >= end) [[unlikely]] {
         ctx.error = error_code::unexpected_end;
         return 0;
      ...
      case 3: {
         ...
         if constexpr (sizeof(std::size_t) > sizeof(std::uint32_t)) {
            ...
            it += 8;
            h = h >> 2;
            static constexpr std::uint64_t safety_limit = 1ull << 48; // 2^48
