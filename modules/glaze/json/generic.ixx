...
export namespace glz
{
   ...
   enum class num_mode {
      ...
      i64, // std::int64_t → double - signed integer precision
      u64 // std::uint64_t → std::int64_t → double - full integer range
   };
   ...
   struct generic_json
   {
      ...
      // - i64: std::int64_t, then double (signed integer precision)
      // - u64: std::uint64_t, then std::int64_t, then double (full integer range)
      ...
      // Check if the number is stored as std::uint64_t (only available in u64 mode)
   ...
   using generic_i64 = generic_json<num_mode::i64>; // std::int64_t → double - signed integer precision
   using generic_u64 = generic_json<num_mode::u64>; // std::uint64_t → std::int64_t → double - full integer range
