...
// Tests marked "known failure" exercise the parser but do not assert correctness.
// As YAML support improves, convert known failures to normal assertions.
...
// ============================================================================
// KNOWN FAILURES (234 tests) - Parser is exercised but no assertions.
...
- !e!tag%21 baz
   ...
   // 6FWR (known failure): Block Scalar Keep
   "6FWR"_test = [] {
      std::string yaml = R"yaml(--- |+
 ab
...
Not indented:
 ...
 Flow style: [    # Leading spaces
  ...
  Also by two,    # are neither
   ...
   // 6JQW (known failure): Spec Example 2.13. In literals, newlines are preserved
   "6JQW"_test = [] {
...
--- |
  ...
  // ||  ||__
   ...
   // 6JWB (known failure): Tags for Block Objects
   "6JWB"_test = [] {
...
--- "foo"
 ...
 What a year!
   ...
   // 6WLZ (known failure): Spec Example 6.18. Primary Tag Handle [1.3]
   "6WLZ"_test = [] {
...
!!str 23: !!bool false
   ...
   // 7BMT (known failure): Node and Mapping Key Anchors [1.3]
   "7BMT"_test = [] {
      std::string yaml = R"yaml(---
top1: &node1
  &k1 key1: one
...
single: pair,
   ...
   // 8XYN (known failure): Anchor with unicode character
   "8XYN"_test = [] {
      std::string yaml = R"yaml(---
- &😁 unicode anchor
)yaml";
...
- bla]keks: foo
   ...
   // B63P (known failure): Directive without document
   "B63P"_test = [] {
      std::string yaml = R"yaml(%YAML 1.2
...
)yaml";
...
invalid: x
   ...
   // BEC7 (known failure): Spec Example 6.14. “YAML” directive
   "BEC7"_test = [] {
...
word2
   ...
   // BU8L (known failure): Node Anchor and Tag on Seperate Lines
   "BU8L"_test = [] {
...
"empty":
   ...
   // C4HZ (known failure): Spec Example 2.24. Global Tags
   "C4HZ"_test = [] {
...
- !label
  ...
  text: Pretty vector drawing.
   ...
   // CC74 (known failure): Spec Example 6.20. Tag Handles
   "CC74"_test = [] {
      std::string yaml = R"yaml(%TAG !e! tag:example.com,2000:app/
---
!e!foo "bar"
   ...
   // CN3R (known failure): Various location of anchors in flow sequence
   "CN3R"_test = [] {
...
word2
   ...
   // F2C7 (known failure): Anchors and Tags
   "F2C7"_test = [] {
...
fifteen: d
   ...
   // GT5M (known failure): Node anchor in sequence
   "GT5M"_test = [] {
      std::string yaml = R"yaml(- item1
&node
- item2
...
---
   ...
   // PW8X (known failure): Anchors on Empty Scalars
   "PW8X"_test = [] {
      std::string yaml = R"yaml(- &a
- a
-
...
mapping:
  ...
  sea : green
   ...
   // SKE5 (known failure): Anchor before zero indented sequence
   "SKE5"_test = [] {
      std::string yaml = R"yaml(---
seq:
 &anchor
...
!yaml!str "foo"
   ...
   // U3XV (known failure): Node and Mapping Key Anchors
   "U3XV"_test = [] {
      std::string yaml = R"yaml(---
top1: &node1
  &k1 key1: one
...
...
   ...
   // W5VH (known failure): Allowed characters in alias
   "W5VH"_test = [] {
...
block: > # lala
  abcde
   ...
   // Y2GN (known failure): Anchor with colon in the middle
   "Y2GN"_test = [] {
      std::string yaml = R"yaml(---
key: &an:chor value
)yaml";
...
folded: !foo >1
 value
   ...
   // Z9M4 (known failure): Spec Example 6.22. Global Tag Prefix
   "Z9M4"_test = [] {
      std::string yaml = R"yaml(%TAG !e! tag:example.com,2000:app/
---
- !e!foo "bar"
