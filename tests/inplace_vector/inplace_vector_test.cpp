...
suite capacity_tests = [] {
   ...
   "reserve_and_shrink_to_fit"_test = [] {
      ...
      expect(throws([&v] {
         v.reserve(20);
      })) << "reserve should throw if n > capacity\n";
   };
   ...
   "capacity_exceeded"_test = [] {
      ...
      expect(throws([&v] {
         v.push_back(4);
      })) << "push_back should throw if capacity would be exceeded\n";
      
      expect(throws([&v] {
         v.resize(4);
      })) << "resize should throw if capacity would be exceeded\n";
      
      expect(throws([&v] {
         v.insert(v.begin(), 0);
      })) << "insert should throw if capacity would be exceeded\n";
   };
};
...
suite element_access_tests = [] {
   ...
   "at"_test = [] {
      ...
      expect(throws([&v] {
         v.at(10);
      })) << "at() should throw for out-of-bounds access\n";
   };
...
suite edge_cases_tests = [] {
   "zero_capacity"_test = [] {
      ...
      expect(throws([&v] {
         v.push_back(1);
      })) << "push_back on vector with zero capacity should throw\n";
