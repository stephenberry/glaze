...
private:
    ...
    static X509* create_certificate(EVP_PKEY* pkey, const std::string& subject, int days = 365) {
        ...
        if (subject == "localhost") {
            ...
            ext = X509V3_EXT_conf_nid(nullptr, &ctx, NID_key_usage,
                const_cast<char*>("keyEncipherment,digitalSignature"));
            if (ext) {
                X509_add_ext(x509, ext, -1);
                X509_EXTENSION_free(ext);
            }
    ...
    static bool write_pem_file(const std::string& filename, 
                              std::function<int(BIO*)> write_func) {
...
public:
    static bool generate_test_certificates() {
        ...
        std::unique_ptr<EVP_PKEY, decltype(&EVP_PKEY_free)> server_key(
            generate_rsa_key(2048), EVP_PKEY_free);
        ...
        std::unique_ptr<X509, decltype(&X509_free)> server_cert(
            create_certificate(server_key.get(), "localhost", 365), X509_free);
        ...
        bool cert_written = write_pem_file("test_cert.pem", [&](BIO* bio) {
            return PEM_write_bio_X509(bio, server_cert.get());
        });
        ...
        std::unique_ptr<EVP_PKEY, decltype(&EVP_PKEY_free)> client_key(
            generate_rsa_key(2048), EVP_PKEY_free);
        ...
        std::unique_ptr<X509, decltype(&X509_free)> client_cert(
            create_certificate(client_key.get(), "test-client", 365), X509_free);
        ...
        bool client_cert_written = write_pem_file("test_client_cert.pem", [&](BIO* bio) {
            return PEM_write_bio_X509(bio, client_cert.get());
        });
    ...
    static bool verify_certificate_files() {
        ...
        std::string cert_content((std::istreambuf_iterator<char>(cert)),
                                std::istreambuf_iterator<char>());
        std::string key_content((std::istreambuf_iterator<char>(key)),
                               std::istreambuf_iterator<char>());

        return cert_content.find("BEGIN CERTIFICATE") != std::string::npos &&
               cert_content.find("END CERTIFICATE") != std::string::npos &&
               key_content.find("BEGIN") != std::string::npos &&
               key_content.find("END") != std::string::npos;
    }
};
...
private:
    void setup_routes() {
        ...
        server_.post("/echo", [](const glz::request& req, glz::response& res) {
            ...
            echo_res.timestamp = std::to_string(std::chrono::duration_cast<std::chrono::seconds>(
        ...
        server_.get("/users", [this](const glz::request&, glz::response& res) {
            res.json(users_);
        });
        ...
        server_.get("/users/{id}", [this](const glz::request& req, glz::response& res) {
            ...
            auto user_it = std::find_if(users_.begin(), users_.end(),
                [user_id](const TestUser& u) { return u.id == user_id; });
...
suite server_lifecycle_tests = [] {
    ...
    "https_server_configuration"_test = [] {
        ...
        auto& server_ref = server.enable_cors()
                                 .set_ssl_verify_mode(0);
...
suite stress_tests = [] {
    ...
    "route_registration_stress"_test = [] {
        ...
        for (int i = 0; i < 50; ++i) {
            ...
            server.get(path, [i](const glz::request&, glz::response& res) {
                res.json(glz::json_t{"test_id", i});
            });
        }
...
int main() {
    ...
    std::cout << "  curl -k -X POST https://localhost:8443/echo -H 'Content-Type: application/json' -d '{\"message\":\"test\",\"echo_count\":3}'\n";
