...
boost::ut::suite valid_vector_test_cases_server = [] {
   ...
   for (const auto& pair : valid_requests) {
      ...
      boost::ut::test(test_name) = [&server, &raw_json, stripped]() {
         std::string response = server.call(raw_json);
         if (stripped->empty()) { // if no id is supplied expected response should be none
            boost::ut::expect(response.empty());
            return;
         }
         boost::ut::expect(response == *stripped);
      };
   }
};

boost::ut::suite vector_test_cases = [] {
   using vec_t = std::vector<int>;
   ...
   boost::ut::test("sum_result = 6") = [&server, &client] {
      bool called{};
      auto request_str = client.request<"summer">(
         1, std::vector{1, 2, 3}, [&called](glz::expected<int, rpc::error> value, rpc::id_t id) -> void {
            called = true;
            boost::ut::expect(value.has_value());
            boost::ut::expect(value.value() == 6);
            boost::ut::expect(std::holds_alternative<std::int64_t>(id));
            boost::ut::expect(std::get<std::int64_t>(id) == std::int64_t{1});
         });
      boost::ut::expect(request_str.first == R"({"jsonrpc":"2.0","method":"summer","params":[1,2,3],"id":1})");

      [[maybe_unused]] auto& requests = client.get_request_map<"summer">();
      boost::ut::expect(requests.size() == 1);
      boost::ut::expect(requests.contains(1)); // the id is 1

      server.on<"summer">([](const vec_t& vec) {
         boost::ut::expect(vec == std::vector{1, 2, 3});
         int sum{std::reduce(std::cbegin(vec), std::cend(vec))};
         return sum;
      });
      std::string response = server.call(request_str.first);
      boost::ut::expect(response == R"({"jsonrpc":"2.0","result":6,"id":1})");

      client.call(response);
      boost::ut::expect(called);
   };
};
...
boost::ut::suite struct_test_cases = [] {
   ...
   boost::ut::test("valid foo request") = [&server, &client] {
      bool called{};
      auto request_str{
         client.request<"foo">("42", foo_params{.foo_a = 1337, .foo_b = "hello world"},
                               [&called](glz::expected<foo_result, rpc::error> value, rpc::id_t id) -> void {
                                  called = true;
                                  boost::ut::expect(value.has_value());
                                  boost::ut::expect(value.value() == foo_result{.foo_c = true, .foo_d = "new world"});
                                  boost::ut::expect(std::holds_alternative<std::string_view>(id));
                                  boost::ut::expect(std::get<std::string_view>(id) == "42");
                               })};
      boost::ut::expect(request_str.first ==
                 R"({"jsonrpc":"2.0","method":"foo","params":{"foo_a":1337,"foo_b":"hello world"},"id":"42"})");

      server.on<"foo">([](const foo_params& params) {
         boost::ut::expect(params.foo_a == 1337);
         boost::ut::expect(params.foo_b == "hello world");
         return foo_result{.foo_c = true, .foo_d = "new world"};
      });

      std::string response = server.call(request_str.first);
      boost::ut::expect(response == R"({"jsonrpc":"2.0","result":{"foo_c":true,"foo_d":"new world"},"id":"42"})");
   ...
   boost::ut::test("valid bar request") = [&server, &client] {
      bool called{};
      auto request_str{client.request<"bar">(
         ...
         [&called](const glz::expected<bar_result, rpc::error>& value, const rpc::id_t& id) -> void {
            called = true;
            boost::ut::expect(value.has_value());
            boost::ut::expect(value.value() == bar_result{.bar_c = true, .bar_d = "new world"});
            boost::ut::expect(std::holds_alternative<std::string_view>(id));
            boost::ut::expect(std::get<std::string_view>(id) == "bar-uuid");
         })};
      boost::ut::expect(request_str.first ==
                 R"({"jsonrpc":"2.0","method":"bar","params":{"bar_a":1337,"bar_b":"hello world"},"id":"bar-uuid"})");

      server.on<"bar">([](const bar_params& params) {
         boost::ut::expect(params.bar_a == 1337);
         boost::ut::expect(params.bar_b == "hello world");
         return bar_result{.bar_c = true, .bar_d = "new world"};
      });

      std::string response = server.call(request_str.first);
      boost::ut::expect(response == R"({"jsonrpc":"2.0","result":{"bar_c":true,"bar_d":"new world"},"id":"bar-uuid"})");
   ...
   boost::ut::test("foo request error") = [&server, &client] {
      bool called{};
      auto request_str{
         client.request<"foo">("42", foo_params{.foo_a = 1337, .foo_b = "hello world"},
                               [&called](glz::expected<foo_result, rpc::error> value, rpc::id_t id) -> void {
                                  called = true;
                                  boost::ut::expect(!value.has_value());
                                  boost::ut::expect(value.error() == rpc::error{rpc::error_e::server_error_lower, "my error"});
                                  boost::ut::expect(std::holds_alternative<std::string_view>(id));
                                  boost::ut::expect(std::get<std::string_view>(id) == "42");
                               })};
   ...
   boost::ut::test("server invalid version error") = [&server] {
      ...
      boost::ut::expect(response_vec.at(0).error.has_value());
      boost::ut::expect(response_vec.at(0).error->code == rpc::error_e::invalid_request);
   };

   boost::ut::test("server method not found") = [&server] {
      server.on<"foo">([](const foo_params&) -> foo_result { return {}; });
      ...
      boost::ut::expect(response_vec.at(0).error.has_value());
      boost::ut::expect(response_vec.at(0).error->code == rpc::error_e::method_not_found);
   };

   boost::ut::test("server invalid json") = [&server] {
      server.on<"foo">([](const foo_params&) -> foo_result { return {}; });
      ...
      boost::ut::expect(
         ...
         << s;
      boost::ut::expect(response_vec.at(0).error.has_value());
      boost::ut::expect(response_vec.at(0).error->code == rpc::error_e::parse_error);
   };

   boost::ut::test("server invalid json batch") = [&server] {
      server.on<"foo">([](const foo_params&) -> foo_result { return {}; });
   ...
   boost::ut::test("server invalid json batch empty array") = [&server] {
      server.on<"foo">([](const foo_params&) -> foo_result { return {}; });
      ...
      boost::ut::expect(s == R"([{"jsonrpc":"2.0","error":{"code":-32600,"message":"Invalid request"},"id":null}])");
      boost::ut::expect(response_vec.at(0).error.has_value());
      boost::ut::expect(response_vec.at(0).error->code == rpc::error_e::invalid_request);
   };

   boost::ut::test("server invalid json illformed batch one item") = [&server] {
      server.on<"foo">([](const foo_params&) -> foo_result { return {}; });
   ...
   boost::ut::test("server invalid json illformed batch three items") = [&server] {
      server.on<"foo">([](const foo_params&) -> foo_result { return {}; });

      auto response_vec = server.call<std::vector<rpc::response_t<glz::raw_json>>>(R"([1,2,3])");

      boost::ut::expect(response_vec.size() == 3);
      boost::ut::expect(
         glz::write_json(response_vec) ==
         R"([{"jsonrpc":"2.0","error":{"code":-32600,"message":"Invalid request","data":"1:1: expected_brace\n   1\n   ^"},"id":null},{"jsonrpc":"2.0","error":{"code":-32600,"message":"Invalid request","data":"1:1: expected_brace\n   2\n   ^"},"id":null},{"jsonrpc":"2.0","error":{"code":-32600,"message":"Invalid request","data":"1:1: expected_brace\n   3\n   ^"},"id":null}])");
      for (auto& response : response_vec) {
         boost::ut::expect(response.error.has_value());
         boost::ut::expect(response.error->code == glz::rpc::error_e::invalid_request);
      }
   };
   ...
   "server weird id values"_test = [&server] {
      ...
      boost::ut::expect(response_vec.size() == 2);
      for (auto& response : response_vec) {
         boost::ut::expect(response.error.has_value());
         boost::ut::expect(response.error->code == glz::rpc::error_e::invalid_request);
      }
   ...
   boost::ut::test("server valid or error return") = [&server] {
   ...
   "client request map"_test = [&client] {
      ...
      map.at("first_call")({}, {});
      boost::ut::expect(first_call);
      boost::ut::expect(!second_call);
      boost::ut::expect(!third_call);
      map.at("second_call")({}, {});
      map.at("third_call")({}, {});
      boost::ut::expect(second_call);
      boost::ut::expect(third_call);
   ...
   "client request timeout"_test = [&client] {
      ...
      auto timeout = [&id, &client]() {
         decltype(auto) map = client.get_request_map<"foo">();
         boost::ut::expect(map.contains(id));
         map.erase(id);
      };
      timeout();
   };
   "client request id needs to be unique"_test = [&client] {
      ...
      boost::ut::expect(!second_insert);

      decltype(auto) map = client.get_request_map<"foo">();
      map.at(id)({}, {});
      boost::ut::expect(first_called);

      map.clear();
   };
   "client notification"_test = [&client] {
      const auto notify_str{client.notify<"foo">(foo_params{})};
      boost::ut::expect(notify_str == R"({"jsonrpc":"2.0","method":"foo","params":{"foo_a":0,"foo_b":""},"id":null})");
   };
   ...
   /*"server individual request parameters error"_test = [&server] {
      ...
      for (auto& response : response_vec) {
         ...
         if (response.error.has_value()) {
            boost::ut::expect(response.error->code == glz::rpc::error_e::invalid_params);
         }
      }
   };*/
};
