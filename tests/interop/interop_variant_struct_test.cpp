...
struct glz::meta<EntityContainer> {
    ...
    static constexpr auto value = glz::object(
        ...
        "set_person", &T::set_person,
        "set_company", &T::set_company,
        "set_product", &T::set_product,
        "get_entity_name", &T::get_entity_name
    );
};
...
int main() {
        ...
        for (uint64_t i = 0; i < 3; ++i) {
            ...
            if (alt_desc) {
                if (alt_desc->index != GLZ_TYPE_STRUCT) {
                    std::cout << "Alternative " << i << " has index " << alt_desc->index << " instead of GLZ_TYPE_STRUCT (" << GLZ_TYPE_STRUCT << ")\n";
                }
    ...
    "variant of structs - visitor pattern"_test = [] {
        ...
        auto name = std::visit([](const auto& entity) -> std::string {
            return entity.name;
        }, var);
        expect(name == "Bob");
        
        var = Company{"MegaCorp", {200, 201}, 5000000.0};
        name = std::visit([](const auto& entity) -> std::string {
            return entity.name;
        }, var);
