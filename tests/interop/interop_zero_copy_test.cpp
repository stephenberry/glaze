...
struct VectorTestStruct {
    ...
    float sum_floats() const {
        return std::accumulate(float_vec.begin(), float_vec.end(), 0.0f);
    }
...
int main() {
    ...
    "zero-copy vector view"_test = [] {
        ...
        // Get zero-copy view
        glz_vector view = glz_vector_view(&test_obj.float_vec, 
            glz::create_type_descriptor<std::vector<float>>());
    ...
    "memory alignment for SIMD"_test = [] {
        ...
        glz_vector view = glz_vector_view(&aligned_vec, 
    ...
    "large vector performance"_test = [] {
        ...
        // Get zero-copy view
        glz_vector view = glz_vector_view(&large_vec, 
            glz::create_type_descriptor<std::vector<double>>());
        ...
        std::cout << "✅ Large vector (1M elements) processed in " 
                  << duration.count() << "ms\n";
    };
    ...
    "zero-copy with complex types"_test = [] {
        ...
        // Get zero-copy view
        auto view = glz_vector_view(&complex_vec, 
    ...
    "push_back through API"_test = [] {
        ...
        glz_vector_push_back(&vec, 
            glz::create_type_descriptor<std::vector<float>>(), &val1);
        glz_vector_push_back(&vec, 
            glz::create_type_descriptor<std::vector<float>>(), &val2);
        glz_vector_push_back(&vec, 
            glz::create_type_descriptor<std::vector<float>>(), &val3);
    ...
    "string vector operations"_test = [] {
        ...
        auto view = glz_vector_view(&string_vec, 
            glz::create_type_descriptor<std::vector<std::string>>());
