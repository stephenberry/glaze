...
class ProductService
{
  ...
  public:
   ProductService()
   {
      products_ = {{1, "Laptop", "High-performance laptop", 999.99, "Electronics", true},
...
class HealthChecker
{
  public:
   HealthStatus get_health_status(const MetricsData& metrics)
   {
      ...
      status.metrics.avg_response_time =
         total_requests > 0 ? (metrics.total_response_time.load() / total_requests) : 0.0;
