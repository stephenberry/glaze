...
// bottom, so each retry re-parses the whole subtree below it.
namespace ambiguous_nest
{
   ...
   struct left
   {
      std::variant<left<N - 1>, right<N - 1>> child{};
   };
   template <int N>
   struct right
   {
   ...
   // Calibrated: unbounded retrying costs ~4x per two levels, so this depth takes ~19s in a debug
   // build without the budget and a flat ~50ms with it. Lowering it narrows that gap.
   inline constexpr int depth = 24;

   struct root
   {
      std::variant<left<depth>, right<depth>> v{};
   };

   inline std::string nest(const std::string_view bottom)
   {
      std::string toml = "v = ";
      for (int i = 0; i < depth; ++i) toml += "{ child = ";
      toml += bottom;
