...
namespace glz::repe
{
   ...
   struct header final
   {
      ...
      struct glaze
      {
         ...
         static constexpr auto value =
            glz::array(&T::version, &T::error, &T::notification, &T::user_data, &T::method, &T::id);
      };
   };
   ...
   bool read_params(Value&& value, auto&& state, auto&& response)
   {
      if constexpr (Opts.format == json) {
         ...
         if (ec) {
            glz::write_ndjson(std::forward_as_tuple(header{.error = true},
   ...
   struct server
   {
      ...
      void on(const sv name, Callback&& callback)
      {
         ...
         if constexpr (N == 2) {
            ...
            methods.emplace(name,
                            procedure{[this, params = Params{}, result = Result{}, callback](auto&& state) mutable {
      ...
      template <class Params, class Result,
                class Callback> // requires std::is_assignable_v<std::function<void()>, Callback>
