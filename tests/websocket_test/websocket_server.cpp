...
int main()
{
   ...
   ws_server->on_open([&clients, &clients_mutex](std::shared_ptr<websocket_connection> conn, const request&) {
      ...
      std::cout << "🔗 WebSocket opened: " << conn->remote_address()
   ...
   ws_server->on_close([&clients, &clients_mutex](std::shared_ptr<websocket_connection> conn) {
      ...
      std::cout << "❌ WebSocket closed: " << conn->remote_address()
   ...
   ws_server->on_error([](std::shared_ptr<websocket_connection> conn, std::error_code ec) {
      std::cout << "🚨 WebSocket error for " << conn->remote_address()
      << ": " << ec.message() << std::endl;
   });
   ...
   server.get("/", [](const request&, response& res) {
      res.content_type("text/html").body(read_file("index.html"));
   });
