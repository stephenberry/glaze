...
template <class... T>
auto to_vector(std::variant<T...>&& v)
{
	return std::variant<std::vector<T>...>{};
}

template <class... T>
auto to_variant_pointer(std::variant<T...>&& v)
{
...
template <csv::is_variant variant_t>
struct recorder
{
	...
	std::vector<std::pair<std::string, std::pair<container_type, void*>>> data;
	...
	void register_variable(const std::string& name, variant_p var)
	{
		std::visit([&](auto&& pointer) { 
			...
			data.emplace_back(name, std::make_pair(container_type{ std::vector<T>{} }, pointer));
		}, var);
