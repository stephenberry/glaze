...
namespace glz
{
   struct socket final
   {
      ...
      bool valid() const { return socket_fd != net::invalid_socket; }
...
#ifdef _WIN32
      u_long mode = 1;
      ioctlsocket(sock.socket_fd, FIONBIO, &mode);
#else
                                                         ...
                                                         size_t timeout_ms)
   {
      ...
      while (total_bytes < sizeof(Header)) {
         ...
         auto [bytes, event] =
            async_recv(sckt, reinterpret_cast<char*>(&header) + total_bytes, sizeof(Header) - total_bytes);
   ...
   [[nodiscard]] inline std::error_code blocking_send(socket& sckt, const std::string_view buffer)
   {
      ...
      while (total_bytes < size) {
         auto bytes =
            ::send(sckt.socket_fd, buffer.data() + total_bytes, glz::net::ssize_t(buffer.size() - total_bytes), 0);
         if (bytes == -1) {
            if (GLZ_SOCKET_ERROR_CODE == e_would_block || GLZ_SOCKET_ERROR_CODE == EAGAIN) {
