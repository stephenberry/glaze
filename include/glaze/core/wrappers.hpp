...
namespace glz
{
   namespace detail
   {
      ...
      inline constexpr decltype(auto) opts_wrapper() noexcept
      {
         return [](auto&& val) {
            ...
            return opts_wrapper_t<V, Bit>{val.*MemPtr};
         };
      }
   ...
   constexpr auto bools_as_numbers = detail::opts_wrapper<MemPtr, option::bools_as_numbers>();

   // Read and write numbers as strings
   template <auto MemPtr>
   constexpr auto quoted_num = detail::opts_wrapper<MemPtr, option::quoted_num>();

   // Read numbers as strings and write these string as numbers
   template <auto MemPtr>
   constexpr auto number = detail::opts_wrapper<MemPtr, option::number>();

   // Write out string like types without quotes
   template <auto MemPtr>
   constexpr auto raw = detail::opts_wrapper<MemPtr, option::raw>();

   // Reads into only existing fields and elements and then exits without parsing the rest of the input
   template <auto MemPtr>
   constexpr auto partial_read = detail::opts_wrapper<MemPtr, option::partial_read>();
