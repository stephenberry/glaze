...
namespace glz
{
   ...
   struct trace
   {
      ...
      void end(const std::string_view name) {
         async(name, 'E');
      }
      
      void duration(const std::string_view name, const char phase) {
      ...
      void async_begin(const std::string_view name) {
         async(name, 'b');
      }
      
      void async_end(const std::string_view name) {
         async(name, 'e');
      }
      
      void async(const std::string_view name, const char phase) {
         auto tnow = std::chrono::steady_clock::now();
         if (!t0) {
            t0 = tnow;
         }
   ...
   {
      using T = trace;
      static constexpr auto value = object(&T::traceEvents);
   };
}
