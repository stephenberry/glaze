   ...
   struct from<MSGPACK, T>
   {
      ...
      GLZ_ALWAYS_INLINE static void op(Value&& value, uint8_t tag, Ctx&& ctx, It& it, const End& end) noexcept
      {
         ...
         if constexpr (N == 0) {
            ...
            if (sv == reflect<enum_decode_type<T>>::keys[0]) {
            ...
            const auto index = decode_hash_with_size<MSGPACK, DT, HashInfo, HashInfo.type>::op(
               sv.data(), sv.data() + sv.size(), sv.size());

            if (index >= N || reflect<DT>::keys[index] != sv) [[unlikely]] {
               ctx.error = error_code::unexpected_enum;
               return;
            }
