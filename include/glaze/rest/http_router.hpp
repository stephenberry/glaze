...
namespace glz
{
   ...
   struct http_router
   {
      ...
      struct RadixNode {
         ...
         std::string to_string() const {
            std::stringstream ss;
            ss << "Node[" << (is_parameter ? "PARAM:" : (is_wildcard ? "WILD:" : ""))
            << segment << ", endpoint=" << (is_endpoint ? "true" : "false")
            << ", children=" << static_children.size()
            << (parameter_child ? "+param" : "")
            << (wildcard_child ? "+wild" : "")
            << ", full_path=" << full_path << "]";
            return ss.str();
         }
   ...
   private:
      ...
      void add_route(http_method method, std::string_view path, handler handle,
         ...
         if (path_str.find(':') == std::string::npos && path_str.find('*') == std::string::npos) {
            ...
            if (method_handlers.find(method) != method_handlers.end()) {
               throw std::runtime_error("Route conflict: handler already exists for " +
                                        std::string(to_string(method)) + " " + path_str);
            }
      ...
      bool match_node(RadixNode* node, const std::vector<std::string>& segments,
                      size_t index, http_method method,
                      std::unordered_map<std::string, std::string>& params,
                      handler& result) const {
         // End of path
         ...
         if (static_it != node->static_children.end()) {
            if (match_node(static_it->second.get(), segments, index + 1,
         ...
         if (node->parameter_child) {
            ...
            if (match_node(node->parameter_child.get(), segments, index + 1,
                           method, params, result)) {
               return true;
            }
         ...
         if (node->is_endpoint) {
            ...
            for (const auto& [method, method_constraints] : node->constraints) {
               ...
               for (const auto& [param, constraint] : method_constraints) {
                  std::cout << indent << "    " << param << ": " << constraint.pattern
