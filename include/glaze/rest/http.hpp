...
namespace glz
{
   ...
   inline std::expected<HttpStatusLine, std::error_code> parse_http_status_line(std::string_view status_line) {
      ...
      std::string_view version = status_line.substr(http_prefix.length(),
                                                    first_space - http_prefix.length());
      
      std::string_view status_code_str = status_line.substr(first_space + 1,
                                                            has_status_message ?
      ...
      if (has_status_message && second_space < status_line.length() - 1) {
         ...
         if (start == status_message.end()) {
            ...
            const auto end = std::find_if(status_message.rbegin(), status_message.rend(),
                                          [](unsigned char ch) { return !std::isspace(ch); }).base();
            status_message = status_message.substr(start - status_message.begin(),
                                                   end - start);
         }
      }
      ...
      std::all_of(version.begin(), version.begin() + dot_pos,
                  [](unsigned char ch) { return std::isdigit(ch); }) &&
      std::all_of(version.begin() + dot_pos + 1, version.end(),
                  [](unsigned char ch) { return std::isdigit(ch); });
      ...
      if (status_code_str.empty() ||
          status_code_str.length() > max_status_code_length ||
      ...
      int status_code;
      const auto [ptr, ec] = std::from_chars(status_code_str.data(),
